...
private:
    ...
    std::vector<int> ids_in_the_order_of_adding_documents_;
    ...
    static bool CheckPresenceControlCharacter(const std::string& text);
    ...
    : stop_words_(MakeUniqueNonEmptyStrings(stop_words)) 
{
    for (const std::string& stop_word : stop_words_) {
        if (CheckPresenceControlCharacter(stop_word)) {
            throw std::invalid_argument(std::string("Invalid characters in stop words"));
        }
    }
}
